...
var loop = func {
	...
	if (gear_prev != gear.getValue()) {
	 ...
	 k = 0;
	 if (running.getValue() == 1) {rpm.setValue(300+2900*elevator.getValue())} else {rpm.setValue(0)};
      }
	...
	if ((running.getValue() == 1) and (gear.getValue() > 0)) { 
	 throttle.setValue(1.25*elevator.getValue()+0.1-rpm.getValue()/2300);
	...
	if ((cranking.getValue() == 1) and ((gear.getValue() == 0) or (clutch.getValue == 1)) and (fuel.getValue()>=0.00000319)) {
	 running.setValue(1);	 
	 cranking.setValue(0);
	}
	...
	if (running.getValue() == 1) {
	 if (fuel.getValue() < 0.00000319) {
	  running.setValue(0);
	  }
	 else {
	  fuel_lev = fuel.getValue();
	  fuel.setValue(fuel_lev - (0.9*elevator.getValue()+0.1)*0.00000319);
	 }
	}

	if (running.getValue() == 1) {
	 if (gear.getValue() == -1) {
	  rpm.setValue(330*speed);
	  rev_throttle.setValue(0.9*elevator.getValue()+0.1-rpm.getValue()/2300);
	...
	mbrake = rpm.getValue()/2300-(1.0*elevator.getValue()+0.25*running.getValue());
...
loop();
aircraft.livery.init("Aircraft/Routemaster/Models/Liveries", "sim/model/livery/name");
